...
function hasSourceChanges() {
    ...
    return changedFiles.some(file => CONFIG.sourceDirectories.some(dir => file.startsWith(dir)));
}
/**
...
function hasTestChanges() {
    const changedFiles = getAllChangedFiles();
    return changedFiles.some(file => CONFIG.testPatterns.some(pattern => file.includes(pattern)));
}
// =================================================================
// REVIEW RULES
// =================================================================
...
function checkCriticalFiles() {
    const changedFiles = getAllChangedFiles();
    const criticalChanges = changedFiles.filter(file => CONFIG.criticalFiles.some(critical => file.includes(critical)));
