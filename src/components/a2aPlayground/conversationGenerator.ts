...
if (!SUPABASE_URL) {
  console.debug('[A2A Playground] VITE_SUPABASE_URL environment variable is not set. API calls will fail.');
}
if (!SUPABASE_ANON_KEY) {
  console.debug('[A2A Playground] VITE_SUPABASE_ANON_KEY environment variable is not set. API calls will fail.');
}
